...
def train(mul_X, mul_X_test, device,args):
    ...
    loss_model = Loss(args.alpha, args.Nlabel, device)
    ...
    total_loss = 0
    ytest_Lab = np.zeros([mul_X_test[0].shape[0] - len(ind_00), args.Nlabel])
    ap_loss = []
    ...
    for epoch in range(int(args.maxiter)):
        ...
        for batch_idx in range(int(np.ceil(num_X / args.batch_size))):
            ...
            # print('cont:',loss_Cont.item())
            # for i in range(len(individual_zs)):
            #     loss_Cont += contrast_loss_views(individual_zs[i],fusion_z,0.5)
            # print(sub_target.shape,fusion_z.shape)
            ...
            optimizer.step()
        # scheduler.step()
        yp_prob = test_DIMV(model, mul_X_test, WE_test, args, device)
        ...
        #         _,linshi_q,_ = model(mul_X_test_batch,we)
        #         tmp_q[idx] = linshi_q
        #         del linshi_q
        #     # del x0,x1,x2,x3,x4,x5,we

        # yp_prob = tmp_q.data.cpu().numpy()
        # yp_prob = np.nan_to_num(yp_prob)
        # print(yp_prob.shape)
        # print('ind_00',ind_00)
...
def test_DIMV(model, mul_X_test, WE_test, args, device):
    model.eval()
    num_X_test = mul_X_test[0].shape[0]
    tmp_q = torch.zeros([num_X_test, args.Nlabel]).to(device)
    index_array_test = np.arange(num_X_test)
...
if __name__ == "__main__":
    ...
    file_path = 'final-DIMWLC-' + args.dataset + '_BS_' + str(args.batch_size) + '_VMR_' + str(
    ...
    pre_alpha = [0.2,0.4,0.6,0.8,1] # no lager than 1e-2 [1e-5,1e-4,1e-3]
    pre_beta = [5e-5,1e-4,5e-4,1e-3,5e-3] #[1e-5,1e-3,1e-1]
                ...
                for alpha in pre_alpha:
                    ...
                    for beta in pre_beta:
                        ...
                        for fnum in range(Pre_fnum):
                            ...
                            clum = abs(yt_label).sum(1)
                            ind_00 = np.array(np.where(clum == 0)).reshape(-1)
                            yt_label = np.delete(yt_label, ind_00, axis=0)

                            model, value_result,all_results = train(mul_X, mul_X_test, device,args)
